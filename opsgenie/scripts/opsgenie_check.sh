...
opsgenie_info_text=/tmp/.opsgenie_info.md
...
if [[ "${clang_action}" == "task" ]]; then
    if [[ "${opsgenie_info_silent_send}" == "0" ]]; then
...
task)
    [ "${opsgenie_info_logger}" == "1" ] && logger "[opsgenie]: 启动定时推送任务！"
    echo "本次信息推送：定时任务." >${psgenie_info_text}
    ;;
*)
    [ "${opsgenie_info_logger}" == "1" ] && logger "[opsgenie]: 启动手动推送任务！"
    echo "本次信息推送：手动推送." >${opsgenie_info_text}
    ;;
esac
...
send_title=$(echo "$opsgenie_config_name" | base64_decode)
# 系统运行状态
if [ "${opsgenie_info_system}" == "1" ]; then
    echo "---" >>${opsgenie_info_text}
    ...
    echo "**系统运行状态:**" >>${opsgenie_info_text}
    echo "系统名称: ${router_name}" >>${opsgenie_info_text}
    echo "系统当前时间: ${router_time}" >>${opsgenie_info_text}
    echo "固件版本: ${router_firmware}" >>${opsgenie_info_text}
    echo "系统模式: ${router_mode}" >>${opsgenie_info_text}
    echo "系统运行时间: ${router_uptime}" >>${opsgenie_info_text}
    echo "平均负载(1,5,15min): ${router_cpu_loadavg_1min}, ${router_cpu_loadavg_5min}, ${router_cpu_loadavg_15min}" >>${opsgenie_info_text}
    echo "总内存: ${router_memtotal}MB   空闲数: ${router_memfree}MB" >>${opsgenie_info_text}
    echo "虚拟内存: $(awk 'BEGIN{printf ("%.2f\n",'${router_swaptotal}'-'${router_swapfree}')}') / ${router_swaptotal} MB" >>${opsgenie_info_text}
    echo "JFFS: ${router_jffs_used} / ${router_jffs_total} MB" >>${opsgenie_info_text}
fi
# 温度传感器
if [[ "${opsgenie_info_temp}" == "1" ]]; then
    echo "---" >>${opsgenie_info_text}
    ...
    echo "**CPU温度状况:**" >>${opsgenie_info_text}
    echo "${router_temperature}" >>${opsgenie_info_text}
fi
# 网络运行状态
if [[ "${opsgenie_info_wan}" == "1" ]]; then
    echo "---" >>${opsgenie_info_text}
    # 网络信息
    ...
    echo "**网络状态信息:**" >>${opsgenie_info_text}
    echo "**WAN0状态信息:**" >>${opsgenie_info_text}
    echo "联机类型: ${router_wan0_proto}" >>${opsgenie_info_text}
    echo "公网IPv4地址: ${router_wan0_ip4}" >>${opsgenie_info_text}
    echo "公网IPv6地址: ${router_wan0_ip6}" >>${opsgenie_info_text}
    echo "WAN口IPv4地址: ${router_wan0_ip}" >>${opsgenie_info_text}
    echo "WAN口DNS地址: ${router_wan0_dns1} ${router_wan0_dns2}" >>${opsgenie_info_text}
    echo "WAN口接收流量: ${router_wan0_rx}" >>${opsgenie_info_text}
    echo "WAN口发送流量: ${router_wan0_tx}" >>${opsgenie_info_text}
    echo "---" >>${opsgenie_info_text}
    ...
    if [ -n "${router_wan1_ifname}" ] && [ -n "${router_wan1_gw}" ]; then
        ...
        echo "**WAN1状态信息:**" >>${opsgenie_info_text}
        echo "联机类型: ${router_wan1_proto}" >>${opsgenie_info_text}
        echo "公网IPv4地址: ${router_wan1_ip4}" >>${opsgenie_info_text}
        echo "公网IPv6地址: ${router_wan1_ip6}" >>${opsgenie_info_text}
        echo "WAN口IPv4地址: ${router_wan1_ip}" >>${opsgenie_info_text}
        echo "WAN口DNS地址: ${router_wan1_dns1} ${router_wan1_dns2}" >>${opsgenie_info_text}
        echo "WAN口接收流量: ${router_wan1_rx}" >>${opsgenie_info_text}
        echo "WAN口发送流量: ${router_wan1_tx}" >>${opsgenie_info_text}
        echo "---" >>${opsgenie_info_text}
    fi
    # ddns状态
    ddns_enable=$(nvram get ddns_enable_x)
    if [[ "${opsgenie_info_wan}" == "1" ]]; then
        echo "**DDNS状态信息:**" >>${opsgenie_info_text}
        echo "DDNS域名: $(nvram get ddns_hostname_x)" >>${opsgenie_info_text}
        echo "---" >>${opsgenie_info_text}
    fi
    # ss状态
    ...
    get_china_status() {
        ...
        if [ "$?" == "0" ]; then
            echo '国内链接 【'${net_check_time}'】 √' >>${opsgenie_info_text}
        else
            echo '国内链接 【'${net_check_time}'】 ×' >>${opsgenie_info_text}
        fi
    }
    ...
    get_foreign_status() {
        ...
        if [ "$?" == "0" ]; then
            echo '国外链接 【'${net_check_time}'】 √' >>${opsgenie_info_text}
        else
            echo '国外链接 【'${net_check_time}'】 ×' >>${opsgenie_info_text}
        fi
    }
    ...
    if [[ "${ss_status}" == "1" ]]; then
        echo "**网络连接信息:**" >>${opsgenie_info_text}
        get_china_status >>${opsgenie_info_text}
        get_foreign_status >>${opsgenie_info_text}
        echo "---" >>${opsgenie_info_text}
    fi
    router_lan_ip=$(nvram get lan_ipaddr_rt)
    echo "路由器管理地址: ${router_lan_ip}" >>${opsgenie_info_text}
    echo "---" >>${opsgenie_info_text}
    ...
    wl_list_tab="|:-|:-|"
    echo "${wl_list_title}" >>${opsgenie_info_text}
    echo "${wl_list_tab}" >>${opsgenie_info_text}
    if [[ "${router_smart_connect}" == "1" ]]; then
        router_ssid=$(nvram get wl_ssid)
        [ -n "${router_ssid}" ] && echo "|SmartConnect%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_ssid}|" >>${opsgenie_info_text}
    else
        if [[ "$(nvram get wl0_bss_enabled)" == "1" ]]; then
            router_wl0_ssid=$(nvram get wl0_ssid)
            [ -n "${router_wl0_ssid}" ] && echo "|2.4G%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_wl0_ssid}|" >>${opsgenie_info_text}
        fi
        if [[ "$(nvram get wl1_bss_enabled)" == "1" ]]; then
            router_wl1_ssid=$(nvram get wl1_ssid)
            [ -n "${router_wl0_ssid}" ] && echo "|5G-1%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_wl1_ssid}|" >>${opsgenie_info_text}
        fi
        if [[ "$(nvram get wl2_bss_enabled)" == "1" ]]; then
            router_wl2_ssid=$(nvram get wl2_ssid)
            [ -n "${router_wl0_ssid}" ] && echo "|5G-2%26nbsp;%26nbsp;%26nbsp;%26nbsp;|${router_wl2_ssid}|" >>${opsgenie_info_text}
        fi
    ...
    wlg_list_tab="|:-|:-|"
    echo "${wlg_list_title}" >>${opsgenie_info_text}
    echo "${wlg_list_tab}" >>${opsgenie_info_text}
    ...
    for wlg_status in ${wlg_lists}; do
        if [[ "$(nvram get wl${wlg_status}_bss_enabled)" == "1" ]]; then
            ...
            echo "|${wlg_name}%26nbsp;%26nbsp;%26nbsp;%26nbsp;|$(nvram get wl${wlg_status}_ssid)|" >>${opsgenie_info_text}
            wlg_en="true"
        fi
    done
    if [[ "${wlg_en}" != "true" ]]; then
        echo "|无访客网络||" >>${opsgenie_info_text}
    fi
    #echo "SSID: ${router_ssid}" >> ${opsgenie_info_text}
fi
# USB设备列表
if [[ "${opsgenie_info_usb}" == "1" ]]; then
    echo "---" >>${opsgenie_info_text}
    echo "**USB设备列表:**" >>${opsgenie_info_text}
    ...
    if [ -n "${usb1_fs_path}" ]; then
        ...
        echo "USB1名称: ${usb1_product}(${usb1_status})" >>${opsgenie_info_text}
        echo "USB1容量: ${usb1_used} / ${usb1_total}" >>${opsgenie_info_text}
        echo "USB1可用容量：${usb1_available}" >>${opsgenie_info_text}
        echo "USB1已用：${usb1_use}" >>${opsgenie_info_text}
    else
        echo "USB1接口下无设备" >>${opsgenie_info_text}
    fi
    echo "---" >>${opsgenie_info_text}
    ...
    if [ -n "${usb2_fs_path}" ]; then
        ...
        echo "USB2名称: ${usb2_product}(${usb2_status})" >>${opsgenie_info_text}
        echo "USB2容量: ${usb2_used} / ${usb2_total}" >>${opsgenie_info_text}
        echo "USB2可用容量：${usb2_available}" >>${opsgenie_info_text}
        echo "USB2已用：${usb2_use}" >>${opsgenie_info_text}
    else
        echo "USB2接口下无设备" >>${opsgenie_info_text}
    fi
fi
# 客户端列表
if [[ "${opsgenie_info_lan}" == "1" ]]; then
    echo "---" >>${opsgenie_info_text}
    echo "**客户端列表:**" >>${opsgenie_info_text}
    arp_list=$(arp | grep "br0" | grep -v "incomplete" | sed 's/ (/\t/g' | sed 's/) at /\t/' | sed 's/\[ether\]/ /g' | cut -d " " -f1 | sort -t "." -k3n,3 -k4n,4)
    #echo "${arp_list}" >${arp_file}
    if [ -n "${arp_list}" ]; then
        if [ "$opsgenie_info_lan_macoff" == "1" ]; then
        ...
        echo "${arp_list_title}" >>${opsgenie_info_text}
        echo "${arp_list_tab}" >>${opsgenie_info_text}
        arp_num=0
        echo "${arp_list}" | while read line; do
            arp_client_mac=$(echo ${line} | awk '{print $3}')
            arp_dhcp_white_name=$(echo $(dbus get opsgenie_trigger_dhcp_white | base64_decode | grep -i "${arp_client_mac}") | cut -d# -f2)
            if [[ "${arp_dhcp_white_name}" == "" ]]; then
                ...
                if [[ "${arp_custom_clientlist}" == "" ]]; then
                    if [ "$opsgenie_info_lan_macoff" == "1" ]; then
                        echo ${line} | awk '{print "|"$2"%26nbsp;%26nbsp;|"$1"|"}' >>${opsgenie_info_text}
                    else
                        echo ${line} | awk '{print "|"$2"%26nbsp;%26nbsp;|"$3"%26nbsp;%26nbsp;|"$1"|"}' >>${opsgenie_info_text}
                    fi
                else
                    arp_lease_name=$(echo ${arp_custom_clientlist} | awk -F "" '{print $1}')
                    if [ "$opsgenie_info_lan_macoff" == "1" ]; then
                        echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"}')"${arp_lease_name}|" >>${opsgenie_info_text}
                    else
                        echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"$3"%26nbsp;%26nbsp;|"}')"${arp_lease_name}|" >>${opsgenie_info_text}
                    fi
                fi
            else
                if [ "$opsgenie_info_lan_macoff" == "1" ]; then
                    echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"}')"${arp_dhcp_white_name}|" >>${opsgenie_info_text}
                else
                    echo $(echo "${line}" | awk '{print "|"$2"%26nbsp;%26nbsp;|"$3"%26nbsp;%26nbsp;|"}')"${arp_dhcp_white_name}|" >>${opsgenie_info_text}
                fi
            fi
            let arp_num=arp_num+1
        done
    else
        echo "路由器下没有找到在线客户端" >>${opsgenie_info_text}
    fi
fi
# DHCP租期列表
if [[ "${opsgenie_info_dhcp}" == "1" ]]; then
    total_lease_client=$(cat ${dnsmasq_leases_file} | wc -l)
    echo "---" >>${opsgenie_info_text}
    echo "现在租约期内的客户端共有 ${total_lease_client} 个,情况如下：" >>${opsgenie_info_text}
    if [ "$opsgenie_info_dhcp_macoff" == "1" ]; then
    ...
    echo "${total_lease_info_title}" >>${opsgenie_info_text}
    echo "${total_lease_info_tab}" >>${opsgenie_info_text}
    ...
    echo "${dnsmasq_leases_list}" | while read line; do
        ...
        if [[ "${trigger_dhcp_white_name}" == "" ]]; then
            ...
            if [[ "${dhcp_custom_clientlist}" == "" ]]; then
                if [ "$opsgenie_info_dhcp_macoff" == "1" ]; then
                    echo ${line} | awk '{print "|"$3"%26nbsp;%26nbsp;|"$4"|"}' >>${opsgenie_info_text}
                else
                    echo ${line} | awk '{print "|"$3"%26nbsp;%26nbsp;|"$2"%26nbsp;%26nbsp;|"$4"|"}' >>${opsgenie_info_text}
                fi
            else
                dhcp_lease_name=$(echo ${dhcp_custom_clientlist} | awk -F "" '{print $1}')
                if [ "$opsgenie_info_dhcp_macoff" == "1" ]; then
                    echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"}')"${dhcp_lease_name}|" >>${opsgenie_info_text}
                else
                    echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"$2"%26nbsp;%26nbsp;|"}')"${dhcp_lease_name}|" >>${opsgenie_info_text}
                fi
            fi
        else
            if [ "$opsgenie_info_dhcp_macoff" == "1" ]; then
                echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"}')"${trigger_dhcp_white_name}|" >>${opsgenie_info_text}
            else
                echo $(echo "${line}" | awk '{print "|"$3"%26nbsp;%26nbsp;|"$2"%26nbsp;%26nbsp;|"}')"${trigger_dhcp_white_name}|" >>${opsgenie_info_text}
            fi
        fi
        let num=num+1
    done
...
opsgenie_send_title="${send_title} 路由器状态信息"
opsgenie_send_content=$(cat ${opsgenie_info_text} | sed 's/$/\\n/' | tr -d '\n')
...
for nu in ${sckey_nu}; do
    ...
    result=$(curl -X POST https://api.opsgenie.com/v2/alerts \
        ...
        -d "{\"message\": \"${opsgenie_send_title}\",  \"note\": \"${opsgenie_send_content}\",  \"tags\": [\"状态信息\", \"${send_title}\"], \"priority\":\"P3\"}")
    if [[ -n "$(echo $result | grep 'Request will be processed')" ]]; then
        [ "${opsgenie_info_logger}" == "1" ] && logger "[opsgenie]: 路由器状态信息推送到 SCKEY No.${nu} 成功！"
    else
        [ "${opsgenie_info_logger}" == "1" ] && logger "[opsgenie]: 路由器状态信息推送到 SCKEY No.${nu} 失败，请检查网络及配置！"
    fi
done
sleep 2
rm -rf ${opsgenie_info_text}
